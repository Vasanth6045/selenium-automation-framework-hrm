...
public class TC_SYS_02 extends BaseClass {
	...
	public void ApplyAndApproveLeaveRequest() throws IOException {
		...
		// Click My Leave Request sub menu
		...
		// Open end date breakdown drop down
		...
		ObjectUtility.getTest().log(Status.INFO, "leave request created successfully");
		...
		Assert.assertTrue(mlrp.getMyLeaveRequestHedder().getText().contains("My Leave Requests"),"my leave request page not displayed");
		ObjectUtility.getTest().log(Status.INFO, "my leave request page displayed");
		//capture text of status in leave table
		String status = wd.findElement(By.xpath("(//div[.='"+dt+"']/ancestor::div[@data-toggle='oh-modal-toggle']/descendant::div[@class='oh-sticky-table__td '])[2]")).getText();
		//verify status changed to approved
		Assert.assertTrue(status.contains("Approved"),"leave approval status not displayed");
		ObjectUtility.getTest().log(Status.INFO, "leave approval status displayed");
